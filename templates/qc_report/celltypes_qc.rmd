...
Marker genes are obtained from the list of Human cell markers on [`CellMarker2.0`](http://www.bio-bigdata.center/CellMarker_download.html).
All marker genes shown were specific to a single cell type, with the exception of hematopoietic precursor cells, if present, which express genes found in other, more differentiated immune cells.
...
Note that dots are only shown if the gene has mean expression > 0 and is expressed in at least 10% of the cells in the given cell type.
  ")
```
...
validation_groups_df <- readr::read_tsv(params$validation_groups_file) |>
...
markers_df <- readr::read_tsv(params$validation_markers_file) |>
...
celltype_colors <- readr::read_tsv(params$validation_palette_file) |>
  tibble::deframe()
```
...
```{r, eval=has_submitter & (has_consensus | has_cellassign | has_singler) & !is_supplemental, results='asis'}
# only show cell type heatmap section if submitter exists and there's something to compare it to 
# this only is included if it's the main report 

# print sentence describing methods based on what methods are available
if(has_consensus){
  comparison_text <- "This section displays heatmaps comparing consensus cell type labels to submitter annotations."
  celltypes_to_plot <- "Consensus"
} else if (has_singler || has_cellassign){
  comparison_text <- "This section displays heatmaps comparing cell type labels from automated methods to submitter annotations."
  if(has_singler & has_cellassign){
    # just in case we have singleR and cellassign but no consensus 
    celltypes_to_plot <- c("SingleR", "CellAssign")
  } else if(has_singler){
    celltypes_to_plot <- "SingleR"
  } else if(has_cellassign){
    celltypes_to_plot <- "CellAssign"
  }
}
...
```{r, eval = has_submitter & (has_cellassign | has_singler | has_consensus) & !is_supplemental}
# calculate matrices comparing to submitter
jaccard_submitter_matrices <- celltypes_to_plot |>
  ...
  purrr::set_names(celltypes_to_plot)
...
```{r, eval = has_submitter & (has_cellassign | has_singler | has_consensus) & !is_supplemental , fig.height = plot_height, fig.width = 8.5, warning = FALSE}
