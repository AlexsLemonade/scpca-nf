...
<!--
This file is meant to be run as a child report from `main_qc_report.rmd` or 
-->
...
```{r}
# remove options that don't make sense to share here
  ...
  purrr::map(
    \(x) {
      if(is.null(x)) {
        ...
        # chr_exclude and min_max_counts_per_cell are vectors, so string them up?
        paste(x, collapse = " ")
      }
    }) |>
  as.data.frame() |>
