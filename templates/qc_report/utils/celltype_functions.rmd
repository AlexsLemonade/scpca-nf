...
#' Check if a given automated cell type method was run or not
#'
#' @param celltype_method The celltype method 
#' @param has_celltype_column Boolean indicating if cell type column is present
#' @param sce_coldata colData slot of a processed SCE object
#'
#' @return TRUE if given method was not run, FALSE if not
check_celltype_method_not_run <- function(
  celltype_method,
  has_celltype_column,
  sce_coldata
) {
  colname <- glue::glue("{tolower(celltype_method)}_celltype_annotation")
  if (!has_celltype_column && colname %in% names(sce_coldata)) {
    # This means the values should all be `"Not run"`
    if (!(all(sce_coldata[[colname]] == "Not run"))) {
      stop(
        glue::glue("Error: Unexpected {celltype_method} celltypes are present.")
      )
    }
    method_not_run <- TRUE
  } else {
    method_not_run <- FALSE
  }

  return(method_not_run)
}
```
