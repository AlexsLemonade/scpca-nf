...
if (has_processed) {
  ...
  has_infercnv_attempt <- !is.null(metadata(processed_sce)$infercnv_success)
  has_infercnv_results <- FALSE
  if (has_infercnv_attempt) {
    has_infercnv_results <- tidyr::replace_na(metadata(processed_sce)$infercnv_success, FALSE) # TRUE or FALSE only
...
```{r, child='infercnv_qc.rmd', eval = has_infercnv_results}
```

<!-- Chunks only included if inferCNV was requested but there are no results -->
```{r eval = has_infercnv_attempt && !has_infercnv_results}
knitr::asis_output("# inferCNV results summary") # this cannot be in a results='asis' chunk
```

```{r results = 'asis', eval = has_infercnv_attempt && !has_infercnv_results}
...
if (is.na(metadata(processed_sce)$infercnv_success) &&
  ...
  stop("Encountered unknown inferCNV condition")
}
```
