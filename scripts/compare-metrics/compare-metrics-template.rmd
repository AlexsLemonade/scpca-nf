...
# convert to tibbles and join
metrics <- list(reference = ref_data, comparison = comp_data) |>
  purrr::map(\(x) {
...
readr::write_rds(metrics, "metrics.rds")
```

```{r}
# temporary chunk for testing
metrics <- readr::read_rds("metrics.rds")
```
...
```{r}
ref_samples <- metrics |>
  dplyr::filter(source == "reference") |>
  dplyr::select(project_id, sample_id, library_id)

comp_samples <- metrics |>
  dplyr::filter(source == "comparison") |>
  dplyr::select(project_id, sample_id, library_id)

# make a table of added and removed
added_samples <- dplyr::anti_join(comp_samples, ref_samples, by = c("project_id", "sample_id", "library_id")) |>
  dplyr::mutate(added_removed = "Added")
removed_samples <- dplyr::anti_join(ref_samples, comp_samples, by = c("project_id", "sample_id", "library_id")) |>
  dplyr::mutate(added_removed = "Removed")

sample_changes <- dplyr::bind_rows(added_samples, removed_samples) |>
  dplyr::arrange(project_id, sample_id, library_id)
```
