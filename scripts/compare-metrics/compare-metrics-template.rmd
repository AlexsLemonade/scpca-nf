...
```{r}
hvg_changes <- metrics_df |>
  dplyr::mutate(
    hvg_identical = purrr::map2_lgl(hv_genes.ref, hv_genes.comp, identical),
    hvg_removed = purrr::map2(hv_genes.ref, hv_genes.comp, setdiff),
    hvg_added = purrr::map2(hv_genes.comp, hv_genes.ref, setdiff),
    hvg_changes = purrr::map_int(hvg_added, length),
    hvg_change_frac = hvg_changes / purrr::map_int(hv_genes.ref, length)
  ) |>
