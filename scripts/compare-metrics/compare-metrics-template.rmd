...
pair_sizes <- function(a_sizes, b_sizes) {
  # find the difference between every pair of a and b,
  # then choose from smallest to largest, eliminating used pairs as you go
  ...
  while (!all(match_df$used)) {
    ...
    # add indexes to result vectors
    pair_count <- pair_count + 1
    new_a[pair_count] <- match_df$a_i[idx]
    new_b[pair_count] <- match_df$b_i[idx]
    # remove the first row and any rows with those indexes
    # Mark used pairs
    used_a <- match_df$a_i == new_a[pair_count]
    used_b <- match_df$b_i == new_b[pair_count]
...
max_ari <- function(a_sizes, b_sizes) {
  ...
  if (sum(a_sizes) != sum(b_sizes)) {
    # if the total number of cells is different, we can't calculate ARI
    return(NA)
  }
  # if the number of clusters is the same, we can just sort the sizes
  if (length(a_sizes) == length(b_sizes)) {
    a_sizes <- sort(a_sizes)
    b_sizes <- sort(b_sizes)
  } else {
    # if the number of clusters is different, we need to pair them
    # to minimize the differences between clusters
...
# filter metrics down libraries in both sets for the remainder of the notebook
...
hvg_stats <- metrics_df |>
  dplyr::mutate(
    ...
    hvg_rank_cor = purrr::map2_dbl(
      hv_genes.ref, hv_genes.comp,
      \(x, y) {
        cor(
          seq_along(x),
          # get the ordering of a elements in b
          as.integer(factor(y, levels = x)),
          use = "complete.obs", # discard
        )
      }
    )
  ) |>
...
ggplot(hvg_plot_df, aes(x = value)) +
  geom_histogram(bins = 20) +
  facet_wrap(vars(metric), ncol = 3, scales = "free_x") +
  labs(
    title = "Histograms of change metrics for highly variable genes",
    x = "",
...
cluster_stats <- metrics_df |>
  dplyr::mutate(
    ...
    cluster_n_change = purrr::map2_int(
      cluster_sizes.ref,
      cluster_sizes.comp,
      \(x, y) {
        length(y) - length(x)
    ...
    cluster_n_change_frac = cluster_n_change / purrr::map_int(cluster_sizes.ref, length),
...
cluster_summary <- cluster_stats |>
  dplyr::summarise(
    ...
    mean_n_change = mean(abs(cluster_n_change[which(!cluster_setequal)])),
    mean_n_change_frac = mean(abs(cluster_n_change_frac[which(!cluster_setequal)])),
    mean_max_ari = mean(cluster_max_ari[which(!cluster_setequal & cluster_n_change == 0)])
  )
...
if (!has_cluster_changes) {
  ...
  knitr::asis_output(glue::glue_data(
    ...
    Among the libraries with changes, the mean change in the number of clusters is **{mean_n_change}**, which corresons  a mean percentage change of **{round(mean_n_change_frac * 100, digits = 2)}%**.

    The mean `maxARI` for the changed clusterings is **{round(mean_max_ari, digits = 2)}**. This is an approximate maximum value calculated based on assuming the best correspondence in sizes among the clusters. Note that this only includes comparisons where the number of clusters is consistent between the two sets.
    "
  ))
}
```
...
```{r, eval=has_cluster_changes}
# cluster change plots
cluster_plot_df <- cluster_changes |>
  dplyr::select(cluster_n_change_frac, cluster_max_ari) |>
  tidyr::pivot_longer(
    cols = everything(),
    names_to = "metric",
    values_to = "value"
...
cluster_plot <- ggplot(cluster_plot_df, aes(x = value)) +
  geom_histogram(bins = 20) +
  facet_wrap(vars(metric), scales = "free_x") +
  labs(
    title = "Histograms of change metrics for clustering",
    x = "",
    y = "Number of libraries"
  ) +
  theme_bw()

suppressWarnings(print(cluster_plot))
```

```{r}
# plot top changed clusters
plot_df <- cluster_changes |>
  dplyr::arrange(desc(cluster_max_ari)) |>
  head(6) |> # only plot the top 6
...
cluster_colors <- palette.colors(
  palette = "Tableau",
  n = length(levels(plot_df$cluster)),
  recycle = TRUE
)

ggplot(plot_df, aes(x = set, y = size, fill = cluster)) +
  geom_col(position = position_stack(reverse = TRUE)) +
  facet_wrap(vars(library_id), scales = "free_y") +
  labs(
    title = "Cluster sizes for the libraries with the largest max ARI changes",
    x = "",
    y = "Cell count",
  ) +
...
cluster_changes |>
  dplyr::select(
    ...
    library_id,
    cluster_n_change,
    cluster_n_change_frac,
    cluster_max_ari
  ) |>
  report_table(
    colnames = c("Project", "Sample", "Library", "Number of clusters changed", "% change", "Max ARI")
  ) |>
  DT::formatPercentage(c("cluster_n_change_frac"), 2) |>
  DT::formatRound(c("cluster_max_ari"), digits = 4)
```
