...
if (has_processed) {
  ...
  if (!is.null(metadata(processed_sce)$infercnv_success)) {
    ...
    # has_infercnv_attempt used when there are no results but a success status
    has_infercnv_attempt <- !has_infercnv_results
...
<!-- Next section only included if inferCNV is present or was attempted -->
```{r, child='infercnv_qc.rmd', eval = has_infercnv_results || has_infercnv_attempt}
```
