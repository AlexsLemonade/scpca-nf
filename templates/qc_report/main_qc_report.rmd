...
params:
  ...
  infercnv_requested: NULL
  infercnv_min_reference_cells: NULL
  infercnv_heatmap_file: NULL
  date: !r Sys.Date()
  seed: NULL
...
library_id <- "SCPCL000001" #params$library
unfiltered_sce <-readRDS("~/ALSF/open-scpca/OpenScPCA-analysis/data/2025-06-30/SCPCP000001/SCPCS000001/SCPCL000001_unfiltered.rds") #params$unfiltered_sce
filtered_sce <-readRDS("~/ALSF/open-scpca/OpenScPCA-analysis/data/2025-06-30/SCPCP000001/SCPCS000001/SCPCL000001_filtered.rds") #params$filtered_sce
processed_sce <-readRDS("~/Desktop/SCPCL000001_processed.rds") #params$processed_sce
...
# if processed, check for umap, celltypes, and infercnv
if (has_processed) {
  ...
  has_infercnv <- "infercnv_table" %in% names(metadata(processed_sce))
...
has_infercnv <- FALSE
processed_sce$is_infercnv_reference <- FALSE
# check if inferCNV is not present but was requested for printing messages
# if requested and we made it this far in the pipeline, there have to be cell types too
infercnv_failed <- has_processed && !has_infercnv && params$infercnv_requested
...
library_information <- tibble::tibble(
  ...
  "Mapping tool" = format(mapping_tool)
)
...
<!-- Next section only included if inferCNV is present -->
```{r, child='infercnv_qc.rmd', eval = has_infercnv}
```

<!-- Chunk only included if inferCNV was requested but is not present -->
```{r, eval = infercnv_failed, results='asis'}
knitr::asis_output('# inferCNV results summary')

# Print message based on cause of "failure": insufficient reference cells, or inferCNV actually failed
if (sum(processed_sce$is_infercnv_reference) < params$infercnv_min_reference_cells) {
  glue::glue("
    <div class=\"alert alert-info\">

    inferCNV was not run because there were not enough normal reference cells present in the processed object.
    {sum(processed_sce$is_infercnv_reference)} reference cells were present, but at least {params$infercnv_min_reference_cells} are required.
    </div>
  ")
} else {
  glue::glue("
    <div class=\"alert alert-info\">
    inferCNV failed to run; no results will be included.
    </div>
  ")
}
```
