...
faceted_umap <- function(umap_df,
  ...
  if (n_celltypes == 1) {
    # If n=1, we do not need to a show a legend
...
```{r}
# Helper function to determine UMAP plot height based on the number of celltypes
determine_umap_height <- function(n_celltypes) {
  dplyr::case_when(
    n_celltypes == 1 ~ 3.5,
    n_celltypes <= 3 ~ 4,
    n_celltypes <= 5 ~ 6,
...
submitter_height <- determine_umap_height(submitter_n_celltypes)
```

```{r, eval = has_submitter && has_umap, message=FALSE, warning=FALSE, fig.height = submitter_height, fig.width = 8}
...
singler_height <- determine_umap_height(singler_n_celltypes)
```


```{r, eval=has_singler && has_umap, message=FALSE, warning=FALSE, fig.height = singler_height, fig.width = 8}
# umap for cell assign annotations
faceted_umap(
  umap_df,
  singler_n_celltypes,
...
cellassign_height <- determine_umap_height(cellassign_n_celltypes)
```

```{r, eval = has_cellassign && has_umap, message=FALSE, warning=FALSE, fig.height = cellassign_height, fig.width = 8}
# umap for cell assign annotations
faceted_umap(
  umap_df,
