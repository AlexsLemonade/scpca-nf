...
# keep the proper names to use for labels of plots
automated_celltypes_available <- automated_celltype_names |>
  purrr::keep(\(x) x %in% metadata(processed_sce)$consensus_celltype_methods) |>
  names()
...
if (scimilarity_not_run) {
  glue::glue(
    "\n\nCell type annotation with `SCimilarity` was attempted but not run due to an insufficient number of cells."
  )
}
...
# we only render this report if there's at least 1 cell
...
In this table, cells labeled "Unknown cell type" are those for which the `OpenScPCA` module was unable to confidently assign an annotation.
Any cells labeled "openscpca-excluded" are those for which the `OpenScPCA` module did not attempt to provide an annotation.
')
...
```{r}
# default sizing for dotplot
dotplot_height <- 8
dotplot_width <- 15
dotplot_text_size <- 11

# update sizing if we have a lot of info to show in the plot
if (has_consensus) {
