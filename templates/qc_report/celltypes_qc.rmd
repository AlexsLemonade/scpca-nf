...
```{r}
# Helper function to determine UMAP plot determine_umap_dimensions based on the number of celltypes
# returns a vector of c(width, height)
determine_umap_dimensions <- function(n_celltypes) {
  dplyr::case_when(
    ...
    n_celltypes <= 5 ~ c(8, 7), # 4 & 5: inset legend
    n_celltypes <= 6 ~ c(8, 7.5), # 6: bottom legend
    .default = c(10, 9.5) # 8 & 9: inset legend
  )
}
```
