...
if (has_cellassign) {
  ...
  organs_string <- stringr::str_flatten_comma(split_organs, last = ", and ")
  ...
  methods_text <- glue::glue(
    ...
    * Annotations from [`CellAssign`](https://github.com/Irrationone/cellassign), a marker-gene-based approach ([Zhang _et al._ 2019](https://doi.org/10.1038/s41592-019-0529-1)).
    Marker genes for cell types categorized to the following organs were obtained from [PanglaoDB](https://panglaodb.se/) and compiled into a reference named `{ref_name}`: {organs_string}.\n"
  )
}
