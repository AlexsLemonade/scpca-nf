...
# this is the case for which we will need `infercnv_min_reference_cells`
...
<!-- Next two sections only included if celltype annotations are present -->
...
<!-- Chunks only included if inferCNV was requested but is not present -->
```{r eval = infercnv_failed}
knitr::asis_output("# inferCNV results summary") # this cannot be in a results='asis' chunk
```

```{r results = 'asis', eval = infercnv_failed}
# this must be in a results='asis' chunk
...
if (sum(processed_sce$is_infercnv_reference) < params$infercnv_min_reference_cells) {
  glue::glue("
    ...
    At least {params$infercnv_min_reference_cells} are required, but only {sum(processed_sce$is_infercnv_reference)} reference cells were present.
    </div>
  ")
} else {
  glue::glue("
