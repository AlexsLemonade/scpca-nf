...
# define all cell type methods considered for use throughout 
all_celltype_methods <- c("singler", "cellassign", "scimilarity", "consensus")
automated_celltype_refs <- c("singler_reference", "cellassign_reference", "scimilarity_model")
...
rank_cor <- function(a, b) {
  ...
  # only calculate correlation if the length is the same, otherwise return NA
...
# Helper function to add a column with cell type changes for each cell type method to metrics_df
# df: the metrics_df being manipulated
# methods: vector of cell type methods to count changes for; by default, all are counted
add_celltype_changes <- function(df, methods = all_celltype_methods) {
  methods |>
    # use reduce to carry updates forward with each iteration
    purrr::reduce( 
      .init = df, # start with df; each map iteration mutates on top of the last
      \(df, method) {
        dplyr::mutate(df,
          "{method}" := purrr::map2(
            !!sym(paste0(method, "_celltypes.ref")),
            !!sym(paste0(method, "_celltypes.comp")),
            celltype_changes
          )
        )
    })
...
if (!has_processing_changes) {
  ...
  glue::glue("
    ...
    There {ifelse(n_libraries == 1, 'is', 'are a total of')} **{n_libraries}** {ifelse(n_libraries == 1, 'library', 'libraries')} with processing changes
...
if (!has_cell_changes) {
  ...
  glue::glue("
    ...
    There {ifelse(n_libraries == 1, 'is', 'are a total of')} **{n_libraries}** {ifelse(n_libraries == 1, 'library', 'libraries')} with cell count changes
    from **{n_projects}** {ifelse(n_projects == 1, 'project', 'projects')}.
  ") |>
    knitr::asis_output()
}
...
if (!has_read_changes && !has_cell_changes) {
  ...
  glue::glue("
    ...
    There {ifelse(n_libraries == 1, 'is', 'are a total of')} **{n_libraries}** {ifelse(n_libraries == 1, 'library', 'libraries')}
...
```{r}
doublet_changes <- metrics_df |>
  dplyr::select(
    project_id,
    sample_id,
    library_id,
    starts_with("scdblfinder_total_doublets")
  ) |>
  tidyr::pivot_longer(
    cols = starts_with("scdblfinder_total_doublets"),
    names_pattern = "(.+)\\.(ref|comp)$",
    names_to = c("field", ".value") # .value separates into `ref` and `comp` fields
  ) |>
  dplyr::filter(is.finite(comp) | is.finite(ref)) |> # keep where at least one is finite
...
changed_doublet_counts <- doublet_changes |>
  dplyr::filter(is.na(change) | change != 0)

has_doublet_changes <- nrow(changed_doublet_counts) > 0
```


```{r}
# doublet summary
if (!has_doublet_changes) {
  knitr::asis_output(
    "There are no changes in the total doublet count between the reference and comparison runs."
  )
} else {
  knitr::asis_output(glue::glue(
    "Of the **{nrow(doublet_changes)}** libraries, **{nrow(changed_doublet_counts)}** {ifelse(nrow(changed_doublet_counts) == 1, 'has', 'have')} changes in total doublet counts."
  ))
}
```

```{r, eval=has_doublet_changes}
# table of doublet changes
report_table(
  changed_doublet_counts,
  summary = TRUE,
  colnames = c("Project", "Sample", "Library", "Reference doublet count", "Comparison doublet count", "Change", "% change")
) |>
  DT::formatPercentage("change_frac", 2)
```
...
```{r, eval=has_doublet_changes}
ggplot(changed_doublet_counts, aes(x = change_frac * 100)) +
  geom_histogram(bins = 20) +
  labs(
    x = "Percent change in doublet count",
...
hvg_summary <- hvg_stats |>
  dplyr::summarise(
    ...
    mean_cor = mean(hvg_rank_cor[which(!hvg_identical)], na.rm = TRUE)
  )

if (!has_hvg_changes) {
  knitr::asis_output(
    "There are no changes in highly variable genes between the reference and comparison runs."
  )
} else {
...
```{r}
# get a list of all celltypes found
...
has_celltypes <- !is.null(all_celltypes) && length(all_celltypes) > 0

if (!has_celltypes) {
  # make sure this is set to false so the chunks don't complain
  has_changed_types <- FALSE
  knitr::asis_output(
    glue::glue(
      "This project does not contain cell types."
    )
  )
}
```
...
```{r, eval=has_celltypes}

# create cell type change dataframe
types_stats <- metrics_df |>
  add_celltype_changes() |>
  dplyr::select(
    project_id,
    sample_id,
    library_id,
    any_of(all_celltype_methods)
  ) |>
  ...
  # make sure everything is a character string otherwise pivot doesn't work
  dplyr::mutate(
    dplyr::across(
  ...
  tidyr::pivot_longer(
    cols = starts_with(all_celltype_methods),
    names_to = c("celltype_method", ".value"),
    names_pattern = glue::glue( 
      "({paste(all_celltype_methods, collapse ='|')})_(.*)"
    )
  ) |>
  dplyr::mutate(
...
if (!has_changed_types) {
  knitr::asis_output("There are no changes in cell type assignments between the reference and comparison runs.")
} else {
