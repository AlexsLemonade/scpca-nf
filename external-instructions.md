...
The command below will pull the `scpca-nf` workflow directly from Github using the `v` version.
...
For example, if you would run `nextflow run AlexsLemonade/scpca-nf -r v`, then you will want to set `-r v` for `get_refs.py` as well to be sure you have the correct containers.
