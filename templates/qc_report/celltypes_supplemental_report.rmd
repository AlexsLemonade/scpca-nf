...
# This variable controls whether to print sentence about referring to the supp
# report, which is run from the `celltypes_qc.rmd` child report. 
# We do not print this sentence here in the supplemental report.
...
<!-- This chunk also defines `available_celltypes` --> 
```{r methods-info-box, eval=TRUE, results = "asis"} 
```


This report contains summarized results from performing cell type annotation for library `r library_id`.
The role of this report is to provide more detailed information about cell type annotation results.
...
This report therefore addresses the following questions:
...
This library has the following cell types:
...
if (has_submitter) {
  methods_text <- glue::glue(
    "{methods_text}
    * Submitter-provided cell type annotations were generated by the original lab which produced this data.\n"
  )
}
