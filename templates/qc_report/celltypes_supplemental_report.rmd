...
To assess the quality of the `CellAsssign`-assigned cell types, we visualize the probability of each cell type annotation. 
These probabilities are provided directly by `CellAssign`:

- `CellAssign` first calculates the probability of each cell being annotated as a cell type present in the reference.
- `CellAssign` then annotates cells by selecting the cell type with the highest robability among all cell types considered. 
- These probabilities range from 0-1.
  - Values closer to 1 indicate that `CellAssign` is most confident in this cell type label compared to other cell types.
...
Blue line segments shown as a 'rug' representing individual values that comprise each distribution.

Note that no distrubution is shown for cell types with 3 or fewer labeled cells, but rug points are shown.
")

# Determine height in inches for plot area and rug based on number of cell types
plot_height <- ceiling(length(unique(celltype_df$cellassign_celltype_annotation))/2.5)
rug_height <- plot_height / 175
```
