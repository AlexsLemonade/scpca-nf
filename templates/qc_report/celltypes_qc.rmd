<!-- Call but do not evaluate text --> 
```{r child = 'celltypes_introduction.rmd'}
```
...
Cell type annotations were obtained as follows: 

<!-- Call chunk to print out text --> 
```{r available_celltypes_text, results='asis', eval=TRUE}
```


<!-- At least one cell type annotation method required for the supplemental report -->
```{r, eval = !is_supplemental & (has_singler | has_cellassign), results='asis'}
glue::glue("
  For additional information about cell typing, including diagnostic plots and heatmap comparisons across cell labels, please refer to the [supplementary cell type QC report](`r params$celltype_report`).
 ")
```
