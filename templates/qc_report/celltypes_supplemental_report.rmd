...
available_celltypes <- c(
  # keep consensus as the first method so it shows up first in comparison plots
...
if (is.null(available_celltypes)) {
  available_celltypes <- character()
}

  
# check if these automated methods were run
cellassign_not_run <- check_celltype_method_not_run(
  "CellAssign",
  has_cellassign,
  colData(processed_sce)
)
scimilarity_not_run <- check_celltype_method_not_run(
  "SCimilarity",
  has_scimilarity,
  colData(processed_sce)
)
...
if (has_openscpca) {
  ...
  glue::glue(
    ...
    * OpenScPCA annotations are assigned as part of the [`OpenScPCA` project](https://openscpca.readthedocs.io/en/latest/).
    The annotations shown here were validated as part of the [`{openscpca_annotation_info$module_name}` analysis module in `OpenScPCA-analysis`]({module_link}) and output from the corresponding Nextflow module in [`OpenScPCA-nf`]({openscpca_link}).
    "
  )
}
...
```{r eval=scimilarity_not_run, results='asis'}
glue::glue("
   <div class=\"alert alert-info\">
   Cell type annotation with [`SCimilarity`](https://genentech.github.io/scimilarity/index.html), a cell type annotation approach that uses a foundation model ([Heimberg _et al._ 2024](https://doi.org/10.1038/s41586-024-08411-y)), was attempted but not run due to an insufficient number of cells.
  </div>
")
```
...
- Cell types with fewer than 3 labeled cells are combined and labeled as 'Other' in the plot.
")
```
...
delta_median_df <- delta_median_df |>
  dplyr::mutate(
    ...
    annotation_wrapped = factor(celltype,
      # rev() so large groups are at the TOP of the plot
      levels = rev(levels(celltype)),
      labels = rev(stringr::str_wrap(levels(celltype), 30))
    ) |>
      # group any celltypes with < 3 cells into "Other"
      forcats::fct_lump_min(3) |>
      forcats::fct_relevel("Other", after = 0)
  )
...
plot_height <- pmax(
  length(unique(delta_median_df$celltype)) / 2.5,
  3
)
```
...
Line segments are taller for any cell type label with 5 or fewer cells.
Cell types with fewer than 3 labeled cells are combined and labeled as 'Other' in the plot.
")

# Determine height in inches for plot area
plot_height <- pmax(
  ceiling(length(unique(celltype_df$cellassign_celltype_annotation)) / 2.5),
  2.5
)
```
...
celltype_df <- celltype_df |>
  ...
  dplyr::mutate(
    cellassign_celltype_annotation = forcats::fct_lump_min(cellassign_celltype_annotation, 3) |>
      forcats::fct_relevel(c("Other", "Unknown cell type"), after = Inf)
  )
...
<!---------------------- Scimilarity  ------------------------->
```{r, eval = has_scimilarity}
knitr::asis_output("
## `SCimilarity` annotations

To assess the quality of `SCimilarity` cell type annotations, we report the minimum distance between the query cell and all cells in the foundational model used for cell type annotation.

- Cells that are similar to cells in the existing model have a lower distance and higher confidence in the model's prediction.
- Cells that are further away from what the model has seen before have a larger distance and lower confidence in the model's prediction.

It's important to note that the distance ranges vary across samples.
As noted in the [`SCimilarity` cell type annotation vignette](https://genentech.github.io/scimilarity/notebooks/cell_annotation_tutorial.html#Annotation-QC), distances larger than 0.05 can indicate that the model is less confident in the prediction.
For more information on the foundational model used and how cell types are assigned, see [Heimberg _et al._ 2024](https://doi.org/10.1038/s41586-024-08411-y).

The plot below shows the distribution of the minimum distance metric reported by `SCimilarity` for each cell with the assigned cell type annotation indicated on the y-axis.
Diamonds represent the median of the minimum distance metric among all cells assigned a given cell type annotation.
Cells to the right of the dashed line are those for which the model is lesonfident in the prediction.
...
scimilarity_df <- celltype_df |>
  ...
  dplyr::mutate(
    annotation_wrapped = factor(scimilarity_celltype_annotation,
      levels = rev(levels(scimilarity_celltype_annotation)),
      labels = stringr::str_wrap(rev(levels(scimilarity_celltype_annotation)), 30)
    ) |>
      # lump celltypes with fewer than 3 cells
      forcats::fct_lump_min(3) |>
      forcats::fct_relevel("Other", after = 0)
  )

# Determine height for plot area based on number of cell types
plot_height <- pmax(
  nlevels(scimilarity_df$annotation_wrapped) / 2.5,
  3
)
```


```{r, eval = has_scimilarity, warning=FALSE, message=FALSE, fig.height = plot_height, fig.width = 8}
# sina plot looking at minimum distance across cells for each cell type annotation
ggplot(scimilarity_df) +
  aes(
    x = scimilarity_min_distance,
    y = annotation_wrapped
  ) +
  ggforce::geom_sina(
    size = 0.8,
  ...
  # add a line for confidence level
