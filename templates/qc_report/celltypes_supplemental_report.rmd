...
To assess the quality of the `CellAsssign`-assigned cell types, we look at the probability associated with the annotated cell type. 
...
Rug points are also taller for any cell type label with 2 or fewer cells.
")
...
y_max <- celltype_df |>
  dplyr::group_by(cellassign_celltype_annotation) |>
  dplyr::select(cellassign_celltype_annotation,
                cellassign_max_prediction) |>
  tidyr::nest() |>
  dplyr::ungroup() |>
  dplyr::mutate(
    density_max_y = purrr::map(
      data, 
      \(x) max(density(x$cellassign_max_prediction, bw = density_bw)$y
    ))
  ) |>
  tidyr::unnest(cols = c(density_max_y)) |>
  dplyr::summarize(y_max = max(density_max_y)) |>
  dplyr::pull(y_max)

# add count to celltype_df for setting alpha values
