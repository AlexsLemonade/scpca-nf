...
prepare_submitter_annotation_values <- function(df) {
  ...
  # used for unknowns - strings which might be "unknown"-y
  unknown_strings <- c("na", "n/a", "<na>", "<n/a>", "null", "unknown", " ")
  
  # define variations here rather than lowering the submitter annotations themselves
  unknown_strings <- c(
    unknown_strings,
    stringr::str_to_upper(unknown_strings), 
    stringr::str_to_title(unknown_strings)
  ) |> 
    unique()
  ...
  unknown_levels <- submitter_levels[submitter_levels %in% unknown_strings]
  ...
  # create new levels:
  # all annotations, in order of frequency
  # all unknowns, in order of frequency
  # submitter-excluded, if present
  new_submitter_levels <- c(
    submitter_levels[!(submitter_levels %in% unknown_levels)], 
    unknown_levels
  )
    
  # finally, apply the new levels
  df$submitter_celltype_annotation <- df$submitter_celltype_annotation |>
    # just in case, remove any current levels
    as.character() |>
    factor(levels = new_submitter_levels)
