...
```{r, results='asis'}
methods_bullets <- available_celltypes |>
  # full string for printing out
  stringr::str_replace("Submitter", "Submitter-provided") |>
  # backticks to only these two methods
  stringr::str_replace_all("(SingleR|CellAssign)", "`\\1`") |>
  # make it bullets
...
glue::glue("
  ...
  The following cell type annotations are present for this library:
...
<!--------------------------------- Opening text ------------------------------> 
This report contains summarized results from performing cell type annotation for library `r library_id`.
The role of this report is to provide more detailed information about cell type annotation results, including the following:

- How do results from different cell type annotation methods compare to one another?
- How do submitter-provided cell type annotations, if available, compare to results from `SingleR` and/or `CellAssign`?
- How reliable are `SingleR` and/or `CellAssign` cell type annotations?

Note that the specific contents of this report will depend on which cell type annotations are present.

One reason that ScPCA employs multiple cell type annotation approaches is to offer a chance to compare their annotations.
Performing cell type annotation is an inherently challenging task with high levels of uncertainty.

Therefore, when multiple methods annotate a given cell as the same or similar cell type, this qualitatively indicates that the annotation is more reliable.

Be aware that different cell type annotation methods may assign different labels to the same or similar cell type (e.g., the different string representations `B cell naive` and `Naive B cell`), due to use of different underlying reference datasets.
...
knitr::asis_output("
## `SingleR` vs. `CellAssign` annotations
...
In this section, we assess the quality of cell type annotations using diagnostic plots.
")
```
