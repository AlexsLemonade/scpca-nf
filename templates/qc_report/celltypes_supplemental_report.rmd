...
To assess the quality of `SCimilarity` cell type annotations, we evaluate the minimum distance between the query cell and all cells in the foundational model used for cell type annotation.
...
As noted in the [`SCimilarity` cell type annotation vignette](https://genentech.github.io/scimilarity/notebooks/cell_annotation_tutorial.html#Annotation-QC), the foundational model was used to push the distance between dissimilar cells to > 0.05.
Therefore, distances larger than 0.05 can indicate that the model is less confident in the prediction.
...
Cells to the right of the dashed line are those which the model is less confident in the prediction.
")
```
...
plot_height <- pmax(
  length(unique(scimilarity_df$scimilarity_celltype_annotation)) / 2.5,
  3
)
```
